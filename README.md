...
├── tracked_signals.json        # 信号跟踪记录（自动生成）
...
信号推送后，系统持续跟踪价格变化，触发关键点位时推送更新：

```
📊 信号跟踪更新

📍 Miami | 2026-02-12 | 78-79°F
🎯 止盈触发!
💰 信号价: $0.160 → 当前: $0.260
📈 涨幅: +62.5%
⏱️ 3小时前
...
| 大幅上涨 | 涨幅 ≥ 20% |
| 大幅下跌 | 跌幅 ≥ 20% |
| 市场结算 | 价格达到 $1.00 或 $0.00 |

每种告警每个信号只推送一次，不受 6 小时信号冷却限制。

#### 5. 每日统计（每天 09:00 美东自动推送）
...
💰 平均回报: +42.3%
🏆 最佳: Miami 78-79°F (+316.7%)
📉 最差: Chicago 40°F+ (-100.0%)
...
| 条件 | 值 |
|------|-----|
| 信号去重冷却 | 同一市场 6 小时内不重复推送 |
| 跟踪告警 | 每种类型每个信号只推一次，不受冷却限制 |
| 每日统计 | 每天 09:00（美东）自动推送 |
| 扫描间隔 | 1 小时 |
...
- 所有信号记录在 `tracked_signals.json`
- 每小时扫描时自动更新价格（复用已加载的市场数据，零额外 API 调用）
- 价格触发关键点位时推送 Telegram 告警
- 市场结算（$1/$0）自动标记预报正确/错误
- 24h 未结算的信号标记为过期
- 结算 7 天后自动清理旧记录
- 每日 09:00（美东）推送统计汇总（胜率、回报、最佳/最差信号）
